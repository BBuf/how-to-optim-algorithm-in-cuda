...
![图 1. TK-GEMM 比 PyTorch（调用 cuBLAS）加速，适用于 Llama3-70B 注意力层矩阵形状（N=K=8192）](https://github.com/BBuf/how-to-optim-algorithm-in-cuda/releases/download/mdnice-assets-2026-08-05-1/1c02cbba-f642-4517-baae-f8aebece1834.png)
...
![上图： BF16、FP16、FP8 E4M3 和 FP8 E5M2。为了展示精度差异，每种格式中都显示了与 0.3952 最接近的表示。](https://github.com/BBuf/how-to-optim-algorithm-in-cuda/releases/download/mdnice-assets-2026-08-05-2/5cf0d6d8-e414-41b5-a14a-34633fa34518.png)
...
![](https://github.com/BBuf/how-to-optim-algorithm-in-cuda/releases/download/mdnice-assets-2026-08-05-3/b5321070-1d9c-4630-b7b8-b80a2ce4f490.png)

这两条指令都告诉我们，我们正在将两个FP8 E4M3格式的输入张量相乘，并在F32（32位浮点数）中进行累加。这确认了TK-GEMM kernel确实在利用FP8 Tensor Core，并且lowering正在正确进行。

### 4.0 SplitK 工作分解

![图 4. TK-GEMM 与基础 Triton GEMM TFLOPS 比较，M=1-64](https://github.com/BBuf/how-to-optim-algorithm-in-cuda/releases/download/mdnice-assets-2026-08-05-3/e3e62990-9df5-4d0e-92c3-6ce5d60f698d.png)
...
![上图：NCU 分析器对不同批次大小的 TK-GEMM 的计时，并与 PyTorch（调用 cuBLAS）FP8 和 FP16 进行了比较。](https://github.com/BBuf/how-to-optim-algorithm-in-cuda/releases/download/mdnice-assets-2026-08-05-1/44d86182-df16-47a8-abd9-3058b08818c1.png)
...
![图 5. CPU 启动开销：2.413 毫秒](https://github.com/BBuf/how-to-optim-algorithm-in-cuda/releases/download/mdnice-assets-2026-08-05-1/45269de7-83bc-46a7-b9c4-21be872acec0.png)

从上面可以看出，我们优化后的kernel的大部分wall时间都被即时（JIT）编译开销所主导。为了解决这个问题，我们可以使用CUDA graphs。

![图 6. CUDA 图可视化](https://github.com/BBuf/how-to-optim-algorithm-in-cuda/releases/download/mdnice-assets-2026-08-05-1/0d2c1732-518e-469e-a8a9-89f9b0edc028.png)
...
![图 7. 使用 TK-GEMM 模拟的 Llama3-70B 注意力层](https://github.com/BBuf/how-to-optim-algorithm-in-cuda/releases/download/mdnice-assets-2026-08-05-2/a41164a7-fc44-4eb9-acf2-5d8af6c356a2.png)

为了展示 CUDA Graph的影响，我们随后在玩具注意力层中创建了 TK-GEMM 内核的Graph，并回放了该Graph。下面我们可以看到，内核执行之间的间隔缩短到 6.65 微秒。

![图 8. 使用 TK-GEMM 和 CUDA Graph模拟的 Llama3-70B 注意力层](https://github.com/BBuf/how-to-optim-algorithm-in-cuda/releases/download/mdnice-assets-2026-08-05-3/f0a0a8e6-a2f7-49a1-bd73-e278ec19a466.png)

在实践中，与在不使用CUDA Graphs的模型中简单使用TK-GEMM相比，这种优化将使Llama3-70B中的单个注意力层获得6.4倍的加速。

### 6.0 潜在的未来优化路径

![](https://github.com/BBuf/how-to-optim-algorithm-in-cuda/releases/download/mdnice-assets-2026-08-05-2/5e586db5-ac7b-4eeb-a2c4-110f230a9ff7.png)

NVIDIA H100 拥有 TMA 硬件单元。专用 TMA 单元释放了寄存器和线程以执行其他工作，因为地址生成完全由 TMA 处理。对于内存受限的问题规模，当 Triton 支持此功能时，这可以提供更大的收益。

![图 10. 张量核心利用率（箭头指示自由度）](https://github.com/BBuf/how-to-optim-algorithm-in-cuda/releases/download/mdnice-assets-2026-08-05-3/fb8e8b48-d6f1-40ed-974f-fe762a80d37a.png)
...
![图 11. DRAM 吞吐量（圈出），H100 上的 1.65 TB/s 对比峰值 3.35 TB/s（M=16，N=8192，K=8192）](https://github.com/BBuf/how-to-optim-algorithm-in-cuda/releases/download/mdnice-assets-2026-08-05-3/bb0d9d30-e3e5-4ca6-a16f-bb601b90a554.png)
