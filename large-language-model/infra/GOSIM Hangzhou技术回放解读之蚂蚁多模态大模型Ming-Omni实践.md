> 这篇是 GOSIM Hangzhou 里 `蚂蚁多模态大模型实践` 这场分享的回放解读。原始 PDF 已经按页转成 mdnice 图片，正文里每一页 slides 都保留了对应图片；技术页我会尽量落到公开代码，讲清楚这页到底对应什么实现。
...
这场分享讲蚂蚁多模态模型实践，主角是 Ming-Omni。它不是单纯 VLM，而是把图像、视频、音频、文本理解和生成放进一个更统一的模型体系里。
