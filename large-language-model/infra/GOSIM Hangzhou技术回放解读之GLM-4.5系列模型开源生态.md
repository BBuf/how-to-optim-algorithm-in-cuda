...
模型结构页通常会讲 MoE、active 参数、上下文长度等。具体参数以官方 GLM-4.5 仓库为准，这里不凭 slides 猜实现细节。
...
slime 文档入口是这里的代码落点。slime README 明确写到它是 GLM-4.5 背后的 RL framework，并且用 Megatron + SGLang 连接训练和 rollout。
...
论文和 GitHub 页给出入口。代码讲解选择 slime，因为它和 GLM-4.5 RL 训练关系最直接。
