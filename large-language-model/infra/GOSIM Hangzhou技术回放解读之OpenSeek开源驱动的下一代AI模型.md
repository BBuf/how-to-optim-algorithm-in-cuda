...
OpenSeek 这组 slides 偏项目和算法路线，但里面有一个很具体的系统点：DMA/flash-dmattn。文章会先按 slides 讲项目，再把 sparse attention 代码落下来。
...
效率方向开始进入代码重点。长上下文 attention 的 O(N^2) 成本会限制训练和推理，DMA/NSA 这类 sparse attention 希望把无效 token 的计算跳掉。
