> 这篇按 slides 顺序梳理 OpenSeek 计划，以及其中能落到代码的 Dynamic Mask Attention / flash-dmattn 实现。图片保留为线上链接；涉及技术实现的部分只讨论公开代码和公开文档。
...
标题页不展开。OpenSeek 这组 slides 偏项目和算法路线，但里面有一个很具体的系统点：DMA/flash-dmattn。文章会先按 slides 讲项目，再把 sparse attention 代码落下来。
