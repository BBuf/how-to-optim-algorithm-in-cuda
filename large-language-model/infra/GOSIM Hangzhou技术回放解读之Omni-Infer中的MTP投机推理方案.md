> 这篇是 GOSIM Hangzhou 里 `Omni-Infer 中的 MTP：昇腾亲和的高吞吐投机推理方案` 这场分享的回放解读。原始 PDF 已经按页转成 mdnice 图片，正文里每一页 slides 都保留了对应图片；技术页我会尽量落到公开代码，讲清楚这页到底对应什么实现。
...
这场分享讲的是 Ascend NPU 上的投机推理。Decode 阶段单 token 迭代很容易受带宽和同步开销限制，MTP 一次预测多个后续 token，目标是把串行 decode 步合并成更适合 NPU 执行的大步。
