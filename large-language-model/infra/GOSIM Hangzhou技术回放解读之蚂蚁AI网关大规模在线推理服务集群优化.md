> 这篇按 slides 顺序梳理大规模在线推理服务里的 AI Gateway 调度问题。图片保留为线上链接；涉及技术实现的部分只讨论公开代码、公开文档和能核验的系统设计。
...
标题页不展开。这里讨论的是推理网关，不是普通 HTTP 网关：LLM 请求的成本取决于 prompt 长度、decode 长度、cache 命中、PD 分离状态和后端 batch，因此路由层必须懂一些模型推理语义。
