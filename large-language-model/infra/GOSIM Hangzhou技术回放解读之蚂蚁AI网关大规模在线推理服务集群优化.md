> 这篇是 GOSIM Hangzhou 里 `蚂蚁AI网关 - 大规模在线推理服务集群性能优化实战` 这场分享的回放解读。原始 PDF 已经按页转成 mdnice 图片，正文里每一页 slides 都保留了对应图片；技术页我会尽量落到公开代码，讲清楚这页到底对应什么实现。
...
这场分享讲的是推理网关，不是普通 HTTP 网关。LLM 请求的成本取决于 prompt 长度、decode 长度、cache 命中、PD 分离状态和后端 batch，因此路由层必须懂一些模型推理语义。
