...
来自Nvidia的Kernel Design Agent团队的成员分享的一个prompt模板，感谢：

![vae组件优化例子](https://files.mdnice.com/user/59/db85069f-2e71-44dd-a982-8e9380af1f86.png)

![模型优化例子](https://files.mdnice.com/user/59/9e4d536b-aa3f-44c3-8d15-f577e39d5f31.png)

分别对Vae组件和模型部分应用这种类似的prompt即可展开优化，注意这里用到了Kernel Design Agent里面的2个skill，然后耗时2周就做到了上述30多个模型的Eager下的性能都超越或者持平了Torch Compile。
...
![](https://files.mdnice.com/user/59/0c341376-7dfd-4abe-bc19-1f3d7e3465d9.png)

总的来说，我们已经可以基于Fable加上上面的这种简单Flow让Agent一直优化下去达到所有Diffusion模型都基本持平或者超越Torch Compile的性能。

# 0x3. 总结

我的观点很明确，我们可以在Agent的帮助下彻底放弃 Torch Compile 了，Torch Compile带来的 kernel 优化几乎可以被 Agent 逆向然后复刻，对 overhead 的 reduce 可以被 SGLang 的 Breakable CUDA Graph 替换，没有性能损失并且在真实的serving服务场景里面更加易用。
