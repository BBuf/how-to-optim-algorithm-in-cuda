...
last=len(v)
ax.set_ylim(0,max(880,max(v)+85));ax.set_yticks([0,200,400,600,800]);ax.set_xlim(.6,last+.4)
ax.set_xticks(range(1,last+1),[f'{i:02d}' for i in range(1,last+1)])
...
txt(base,45,716,f'11—{last}  DSpark',17,color=ORANGE,font=TITLE)
txt(base,955,722,'相同随机输入 · 模拟 accept length 目标 5.5',11.5,color=MUTED,ha='right')
for i,p in enumerate(points):
    x=45+(i%3)*310;y=(i//3)*265
    panel(base,x,763+y,290,244,PALE_ORANGE if i%2==0 else SAND)
    txt(base,x+17,781+y,f"{i+11:02d}  {p['label']}",14)
    txt(base,x+17,818+y,f"{p['output_tps_median']:.1f}",25,color=ORANGE)
