...
half* A_shared_ptr = A_shared +
                     ...
                     (((int)threadIdx.x) % (32 / 8)) * 8;
// Calculate B matrix pointer in shared memory // 计算B矩阵在共享内存中的指针位置
...
A_shared 的大小 `[16 * (32 + 8)]`，这个大小表示每个线程块处理输入/输出矩阵的16行数据，然后每次迭代处理32个输入通道（也就是K/IC）纬度。额外的8列填充是为了避免共享内存的bank冲突。B_shared的大小` [32 * (N + 8)]`，32对应每次迭代处理的32个输入通道（K/IC维度），N是输出矩阵的Tile的列方向大小，+8的填充同样是为了避免共享内存的bank冲突。

这里分析一下A_shared_ptr的下标，
