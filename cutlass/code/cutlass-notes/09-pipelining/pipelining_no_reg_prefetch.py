...
sources = [os.path.join(current_dir, filename) for filename in ["pipelining_no_reg_prefetch.cu"]]
...
for exp in exps:
    ...
    # Case 1: MM
    _ = lib.pipelining_fp16_fp16_fp16_fp16(a, b, None)
    kernel_output = lib.pipelining_fp16_fp16_fp16_fp32(a, b, None)
    if not ENABLE_PROF:
    ...
    # Case 2: MMA
    if ENABLE_MMA:
        kernel_output = lib.pipelining_fp16_fp16_fp16_fp32(a, b, c.clone())
        if not ENABLE_PROF:
...
for exp in exps:
    ...
    # Case 1: MM
    kernel_output = lib.pipelining_bf16_bf16_bf16_fp32(a, b, None)
    if not ENABLE_PROF:
    ...
    # Case 2: MMA
    if ENABLE_MMA:
        kernel_output = lib.pipelining_bf16_bf16_bf16_fp32(a, b, c.clone())
        if not ENABLE_PROF:
