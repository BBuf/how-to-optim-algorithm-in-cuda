...
sources = [os.path.join(current_dir, filename) for filename in ["warp_specialization_pipeline_api.cu"]]

os.environ["TORCH_CUDA_ARCH_LIST"] = "9.0a"

# Load CUDA extension module
lib = load(
    name="warp_specialization_pipeline",
    sources=sources,
    extra_cuda_cflags=[
        "-O3",
...
for exp in exps:
    M, N, K = exp
    print(f" Exp1, M={M}, N={N}, K={K} ".center(PRINT_LENGTH, "-"))
    ...
    # MM v.s. MMA
    o1 = lib.warp_specialization_fp16_fp16_fp16_fp16(a, b, None) + c
    o2 = lib.warp_specialization_fp16_fp16_fp16_fp16(a, b, c)
    if not ENABLE_PROF:
...
for exp in exps:
    M, N, K = exp
    print(f" Exp2, M={M}, N={N}, K={K} ".center(PRINT_LENGTH, "-"))
    ...
    # Case 1: MM
    kernel_output = lib.warp_specialization_fp16_fp16_fp16_fp32(a, b, None)
    if not ENABLE_PROF:
    ...
    # Case 2: MMA
    if ENABLE_MMA:
        kernel_output = lib.warp_specialization_fp16_fp16_fp16_fp32(a, b, c.clone())
        if not ENABLE_PROF:
...
for exp in exps:
    M, N, K = exp
    print(f" Exp3, M={M}, N={N}, K={K} ".center(PRINT_LENGTH, "-"))
    ...
    # Case 1: MM
    kernel_output = lib.warp_specialization_bf16_bf16_bf16_fp32(a, b, None)
    if not ENABLE_PROF:
    ...
    # Case 2: MMA
    if ENABLE_MMA:
        kernel_output = lib.warp_specialization_bf16_bf16_bf16_fp32(a, b, c.clone())
        if not ENABLE_PROF:
