...
sources = [os.path.join(current_dir, filename) for filename in ["dynamic_mma.cu"]]
...
for exp in exps:
    ...
    # Case 1: MM
    _ = lib.dynamic_mma_fp16_fp16_fp16_fp16(a, b, None)
    kernel_output = lib.dynamic_mma_fp16_fp16_fp16_fp32(a, b, None)
    if not ENABLE_PROF:
    ...
    # Case 2: MMA
    kernel_output = lib.dynamic_mma_fp16_fp16_fp16_fp32(a, b, c.clone())
    if not ENABLE_PROF:
...
for exp in exps:
    ...
    # Case 1: MM
    kernel_output = lib.dynamic_mma_bf16_bf16_bf16_fp32(a, b, None)
    if not ENABLE_PROF:
        torch_output = torch.matmul(a.float(), b.T.float()).bfloat16()
        compare_matrix(kernel_output, torch_output)

    # Case 2: MMA
    kernel_output = lib.dynamic_mma_bf16_bf16_bf16_fp32(a, b, c.clone())
    if not ENABLE_PROF:
