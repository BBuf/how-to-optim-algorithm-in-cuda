...
sources = [os.path.join(current_dir, filename) for filename in ["warpgroup_mma_multicast.cu"]]

os.environ["TORCH_CUDA_ARCH_LIST"] = "9.0a"

# Load CUDA extension module
lib = load(
    name="warpgroup_mma_multicast",
    sources=sources,
    extra_cuda_cflags=[
        "-O3",
...
for exp in exps:
    ...
    # Case 2: MMA
    if ENABLE_MMA:
        kernel_output = lib.warpgroup_mma_fp16_fp16_fp16_fp32(a, b, c.clone())
        if not ENABLE_PROF:
...
for exp in exps:
    ...
    # Case 1: MM
    kernel_output = lib.warpgroup_mma_bf16_bf16_bf16_fp32(a, b, None)
    if not ENABLE_PROF:
    ...
    # Case 2: MMA
    if ENABLE_MMA:
        kernel_output = lib.warpgroup_mma_bf16_bf16_bf16_fp32(a, b, c.clone())
        if not ENABLE_PROF:
