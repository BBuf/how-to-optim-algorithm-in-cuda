...
sources = [os.path.join(current_dir, filename) for filename in ["tma_multicast_reduce.cu"]]

os.environ["TORCH_CUDA_ARCH_LIST"] = ".".join(map(str, torch.cuda.get_device_capability()))

# Load CUDA extension module
lib = load(
    name="tma_multicast_reduce",
    sources=sources,
    extra_cuda_cflags=[
        "-O3",
...
for exp in exps:
    ...
    # Case 1: MM
    kernel_output = lib.tma_multicast_reduce_fp16_fp16_fp16_fp32(a, b, None)
    if not ENABLE_PROF:
    ...
    # Case 2: MMA
    if ENABLE_MMA:
        kernel_output = lib.tma_multicast_reduce_fp16_fp16_fp16_fp32(a, b, c.clone())
        if not ENABLE_PROF:
...
for exp in exps:
    ...
    # Case 1: MM
    kernel_output = lib.tma_multicast_reduce_bf16_bf16_bf16_fp32(a, b, None)
    if not ENABLE_PROF:
    ...
    # Case 2: MMA
    if ENABLE_MMA:
        kernel_output = lib.tma_multicast_reduce_bf16_bf16_bf16_fp32(a, b, c.clone())
        if not ENABLE_PROF:
