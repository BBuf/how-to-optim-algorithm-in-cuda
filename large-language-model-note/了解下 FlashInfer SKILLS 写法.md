...
最近看 FlashInfer 仓库的时候发现它的 `.claude/skills/` 目录下维护了三个 SKILL 文件， SKILL 允许开发者在项目仓库中放置结构化的指引文档（通常是 `SKILL.md` 文件），供 AI 代码助手在执行任务时读取和遵循。它的核心思路是：将项目特有的开发流程、调试方法、最佳实践等知识编码成文档，这样当开发者在 Claude Code/Cursor 中让 AI 帮忙完成相关任务时，AI 会先读取对应的 SKILL 文件，然后按照其中的步骤来执行，而不是靠通用知识去猜测。
