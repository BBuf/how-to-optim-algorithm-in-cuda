...
def enable_symm_mem_for_group(group_name: str) -> None:
    ...
    Args:
        group_name (str): 进程组的名称。
    """
    # 如果该进程组已经启用了对称内存,则直接返回
    ...
    # 不同的子组即使名称相同也应该使用不同的存储,所以用排名字符串区分
...
def is_symm_mem_enabled_for_group(group_name: str) -> bool:
    ...
    Returns:
        bool: 如果进程组启用了对称内存则返回True,否则返回False。
            当处于测试模式(_is_test_mode为True)时也会返回True。
    """
    # 如果是测试模式或者进程组名称在存储映射字典中,则表示启用了对称内存
...
def get_symm_mem_workspace(group_name: str, min_size: int) -> _SymmetricMemory:
    """
    获取与进程组关联的对称内存工作区。如果min_size大于group_name关联的工作区大小,
    工作区将被重新分配并重新会合。

    Args:
        group_name (str): 进程组的名称。
        min_size (int): 工作区所需的最小字节数。

    Returns:
        _SymmetricMemory: 与进程组关联的对称内存工作区。
    """
    # 为进程组启用对称内存
    ...
    if tensor is None or size < min_size:
        ...
        tensor = _SymmetricMemory.empty_strided_p2p(
            (max(size, min_size),),  # 使用max确保新空间不小于原空间
            [1],  # stride为1
            torch.uint8,  # 数据类型为uint8
            torch.device(f"cuda:{torch.cuda.current_device()}"),  # 在当前CUDA设备上创建
            group_name,  # 关联的进程组名称
        )
        # 更新映射字典
...
def _pipelined_multi_all_gather_and_consume(
    shard: list[torch.Tensor],  # 输入的分片张量列表
    shard_consumer: Callable[[list[torch.Tensor], int], None],  # 处理分片的回调函数
    ag_out: list[torch.Tensor],  # 输出的全收集结果张量列表
    group_name: str,  # 进程组名称
    ag_out_needed: bool = True,  # 是否需要输出全收集结果
) -> None:
    """
    执行以下逻辑,使用微流水线实现计算和通信重叠:
...
def _pipelined_all_gather_and_consume(
    shard: torch.Tensor,  # 输入的分片张量
    shard_consumer: Callable[[torch.Tensor, int], None],  # 处理分片的回调函数,接收分片和rank作为参数
    ag_out: torch.Tensor,  # 用于存储all-gather结果的输出张量
    group_name: str,  # 进程组名称
    ag_out_needed: bool = True,  # 是否需要保存all-gather的输出结果
) -> None:
    """
    使用微流水线计算和通信执行以下逻辑:

        ag_out = all_gather_tensor(shard, gather_dim=0, group=group)  # 收集所有分片
        shards = ag_out.chunk(group.size())  # 将结果分块
        for src_rank, shard in enumerate(shards):  # 对每个分片执行消费者函数
            shard_consumer(shard, src_rank)
    """

    # 定义适配器函数,将单个张量的消费者函数转换为处理张量列表的函数
    def adapter(shard: list[torch.Tensor], rank: int) -> None:
        shard_consumer(shard[0], rank)

    # 调用多张量版本的流水线all-gather和消费函数
